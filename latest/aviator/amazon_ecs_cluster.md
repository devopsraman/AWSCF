AmazonEcsCluster FAWS Template
==============================
amazon_ecs_cluster.template - Autoscaling EC2 Instances with Rolling Updates and Notifications. **WARNING** This template creates one or more Amazon EC2  instances and an Elastic Load Balancer. You will be billed for the AWS resources used if you create a stack from this template.
### Parameters
...
#### MemAlarmHigh
- Description: `{'Fn::Sub': 'Scale-up if Memory${CwMemHighOperator}${CwMemHighThreshold}% for ${CwMemHighPeriod} seconds ${CwMemHighEvaluations} times.'}`
- Metric: MemoryReservation
- Threshold: `{'Ref': 'CwMemHighThreshold'}`
- Actions: `{'Ref': 'EC2ScaleUpPolicy'}`

#### ScaleAlarmHigh
- Description: `{'Fn::Sub': 'Scale-up if CPUReservation${CwHighOperator}${CwHighThreshold}% for ${CwHighPeriod} seconds ${CwHighEvaluations} times'}`
- Metric: CPUReservation
- Threshold: `{'Ref': 'CwHighThreshold'}`
- Actions: `{'Ref': 'EC2ScaleUpPolicy'}`

#### MemAlarmLow
- Description: `{'Fn::Sub': 'Scale-down if Memory${CwMemLowOperator}${CwMemLowThreshold}% for ${CwMemLowPeriod} seconds ${CwMemLowEvaluations} times.'}`
- Metric: MemoryReservation
- Threshold: `{'Ref': 'CwMemLowThreshold'}`
- Actions: `{'Ref': 'EC2ScaleDownPolicy'}`

#### ScaleAlarmLow
- Description: `{'Fn::Sub': 'Scale-down if CPUReservation${CwLowOperator}${CwLowThreshold}% for ${CwLowPeriod} seconds ${CwLowEvaluations} times.'}`
- Metric: CPUReservation
